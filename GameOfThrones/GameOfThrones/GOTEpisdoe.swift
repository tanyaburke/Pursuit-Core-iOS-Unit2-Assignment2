...
class GOTEpisode {
    ...
    static func seasonSections() -> [[GOTEpisode]] {
        ...
        let sortedGOTEpisode = allEpisodes.sorted {$0.season < $1.season}

           let uniqueSeason = Set(sortedGOTEpisode.map{$0.season})

           

           var sections = Array(repeating: [GOTEpisode](), count: uniqueSeason.count)
           

           var currentIndex = 0
           

        var currentSeason = sortedGOTEpisode.first?.season 
           

           for episode in sortedGOTEpisode{
